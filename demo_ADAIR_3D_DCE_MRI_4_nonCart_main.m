%--------------------------------------------------------------------------
%% load data, this demo is for non-Cartesian data only
%--------------------------------------------------------------------------
...
addpath('.\grasp_v2\');
addpath(genpath('.\BM3D_MRI_toolbox\'))
...
for t = 1:num_iter
    ...
    img_pocs = img_pocs + param.E'*(kspace_raw-tmp_k*norm(kspace_raw(:))/norm(tmp_k(:)));
...
%             
            ...
            tmp_ki = param.E*I3;
            if 1
                I11 = I3 + param.E'*(kspace_raw - tmp_ki*norm(kspace_raw(:))/norm(tmp_ki(:)));
%             
            else
                tmp_tv = param.W*I3;
                I11 = I3 + lambda*tmp_tv + param.E'*(kspace_raw - tmp_ki*norm(kspace_raw(:))/norm(tmp_ki(:)));
            end
